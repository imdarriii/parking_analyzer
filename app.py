...
# чтобы не запускать треды несколько раз
...
def process_video(system: ParkingSystem):
    ...
    while system.is_running:
        ...
        if system.learning_phase:
            ...
            cv2.putText(frame, f"ОБУЧЕНИЕ: {progress}%", (20, 50),
        ...
        cv2.putText(frame, f"СВОБОДНО: {free}/{total_spots}", (20, 50),
...
def init_systems_once():
    ...
    with _init_lock:
        ...
        logging.info("🚀 Initializing parking systems (gunicorn-safe)...")
        ...
        threading.Thread(target=process_video, args=(ala,), daemon=True).start()


        # Technopark (на Render файла нет — это нормально)
        if os.path.exists(TECHNOPARK_VIDEO_PATH):
            tech = ParkingSystem(location_id="technopark", source=TECHNOPARK_VIDEO_PATH, auto_learn=True)
            tech.is_running = True
            parking_systems["technopark"] = tech
            threading.Thread(target=process_video, args=(tech,), daemon=True).start()
            logging.info("✓ Технопарк: режим авто-обучения активирован")
        else:
            logging.warning(f"⚠ Видео не найдено: {TECHNOPARK_VIDEO_PATH} (это ожидаемо на Render)")
...
# ИНИЦИАЛИЗАЦИЯ ПРИ ИМПОРТЕ (чтобы работало на Render)
init_systems_once()
...
def get_system_from_request():
    # на всякий случай гарантируем, что инициализация была
    init_systems_once()
